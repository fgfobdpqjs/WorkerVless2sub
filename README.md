...
### 3. 修改 快速订阅入口 以及 添加内置 Vless 节点信息：
  ...
  例如您的pages项目域名为：`sub.fuck.cloudns.biz`；
   ...
   - 添加 `HOST` 变量，例如 `fgfobdpqjs.pages.dev`；
...
### 4. 添加你的专属优选线路：
   ...
   ```url
   https://raw.githubusercontent.com/fgfobdpqjs/WorkerVless2sub/main/addressesapi.txt
   https://raw.githubusercontent.com/fgfobdpqjs/WorkerVless2sub/main/addressesipv6api.txt
   ```
...
<summary><code><strong>「 我不是小白！我有IP库！我知道IPtest是什么！我也有csv测速文件！ 」</strong></code></summary>
   ...
   ```js
   https://raw.githubusercontent.com/fgfobdpqjs/WorkerVless2sub/main/addressescsv.csv
   ```
...
### 1. 部署 Cloudflare Worker：
   ...
   - 将 [worker.js](https://github.com/fgfobdpqjs/WorkerVless2sub/blob/main/_worker.js)  的内容粘贴到 Worker 编辑器中。


### 2. 修改 快速订阅入口 以及 添加内置 Vless 节点信息：

  例如您的workers项目域名为：`sub.fgfobdpqjs.workers.dev`；
   - 添加 `TOKEN` 变量，快速订阅访问入口，默认值为: `auto` ，获取订阅器默认节点订阅地址即 `/auto` ，例如 `https://sub.fgfobdpqjs.workers.dev/auto`
   - 添加 `HOST` 变量，例如 `fgfobdpqjs.pages.dev`；
   - 添加 `UUID` 变量，例如 `30e9c5c8-ed28-4cd9-b008-dc67277f8b02`；
   - 添加 `PATH` 变量，例如 `/?ed=2048`；
 ...
 **3.2 修改 addressesapi 参数示例**
 ...
 - 修改 `addressesapi` 参数，在脚本中设置 `addressesapi` 变量为 **优选IP地址txt文件** 的 URL。例如：
	...
	```
	可参考 [addressesapi.txt](https://raw.githubusercontent.com/fgfobdpqjs/WorkerVless2sub/main/addressesapi.txt) 内容格式 自行搭建。
...
### Trojan订阅
   ...
   - **host**：您的 Trojan 伪装域名，例如 `fgfo.com.mp`；
   - **uuid**：您的 Trojan 客户端 Password，例如 `114514`；
...
| HOST | `fgfobdpqjs.pages.dev` | 快速订阅内置节点的伪装域名 | 
...
| ADDAPI | [https://raw.github.../addressesapi.txt](https://raw.githubusercontent.com/fgfobdpqjs/WorkerVless2sub/main/addressesapi.txt) | 对应`addressesapi`字段 (支持多元素, 元素之间使用`,`作间隔) | 
| ADDNOTLS | `icook.hk:8080#官方优选域名` | 对应`addressesnotls`字段 (支持多元素, 元素之间使用`,`作间隔) | 
| ADDNOTLSAPI | [https://raw.github.../addressesapi.txt](https://raw.githubusercontent.com/fgfobdpqjs/CFcdnVmess2sub/main/addressesapi.txt) | 对应`addressesnotlsapi`字段 (支持多元素, 元素之间使用`,`作间隔) | 
| ADDCSV | [https://raw.github.../addressescsv.csv](https://raw.githubusercontent.com/fgfobdpqjs/WorkerVless2sub/main/addressescsv.csv) | 对应`addressescsv`字段 (支持多元素, 元素之间使用`,`作间隔) | 
...
| SUBCONFIG | [https://raw.github.../ACL4SSR_Online_Full_MultiMode.ini](https://raw.githubusercontent.com/fgfobdpqjs/ACL4SSR/main/Clash/config/ACL4SSR_Online_Full_MultiMode.ini) | clash、singbox等 订阅转换配置文件 | 
| SUBNAME | `WorkerVless2sub` | 订阅生成器名称 | 
| SOCKS5DATA | [https://raw.github.../socks5Data](https://raw.githubusercontent.com/fgfobdpqjs/WorkerVless2sub/main/socks5Data) | Socks5代理池 | 
| PS | `【请勿测速】` | 节点名备注消息 | 
...
## Star 星星走起
[![Stargazers over time](https://starchart.cc/fgfobdpqjs/WorkerVless2sub.svg?variant=adaptive)](https://starchart.cc/cmliu/WorkerVless2sub)
